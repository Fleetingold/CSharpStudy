...
namespace CSharpStudy
{
    internal class HttpClientProgram
    {
        ...
        private static void GetAsync(string requestUri)
        {
            for (int i = 0; i < 10; i++)
            {
                var result = _httpClient.GetAsync(requestUri).GetAwaiter().GetResult();

                string responseContent = result.Content.ReadAsStringAsync().GetAwaiter().GetResult();

                Console.WriteLine($"{i}:{result.StatusCode} Content:{responseContent}");
            }
        }
